...
function NswCalculator() {
  ...
  function handleSubmit() {
    const input: EstimateInput = {
      ...
      local: { ageGroup, feeReliefTier, serviceWeeks, weeklyDays, days },
                  ...
                  return (
                    <ResultCard
                      ...
                      rows={[
                        ...
                        { label: 'Your Estimated Gap', value: fmt(debt.debtPerDay > 0 ? adjustedGap : dailyResult.estimatedGapFee), highlight: true, detail: debt.debtPerDay > 0 ? `${fmt(fee)} – ${fmt(debt.ccsPaidToService)} – ${fmt(dailyResult.dailyFeeRelief)}` : `${fmt(dailyResult.gapBeforeFeeRelief)} – ${fmt(dailyResult.dailyFeeRelief)}` },
                      ]}
                    />
                  )
                })()}
