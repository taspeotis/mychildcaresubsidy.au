...
function QldCalculator() {
  ...
  useEffect(() => {
    ...
    if (estimate.scheme !== 'qld') { cancelEditing(); return }
    ...
    setKindyHours(estimate.local.kindyHours)
    setKindyStart(estimate.local.kindyStart)
    setFnKindyHours(estimate.local.fnKindyHours)
    setFnKindyStart(estimate.local.fnKindyStart)
  ...
  const isEditing = editingEstimate?.scheme === 'qld'
